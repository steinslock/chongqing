"""Shared data loading helpers for Goal 2.7."""
...
def add_clean_demographics(frame: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    out = frame.copy()
    groups_path = project_path(config.get("paths", {}).get("groups_file", ""))
    if groups_path.exists() and "group_code" not in out.columns:
        groups = pd.read_csv(groups_path, dtype={"L_id": str})
        keep = [col for col in ["L_id", "group_code", "group_family", "group_prefix2", "group_prefix3"] if col in groups.columns]
        out = out.merge(groups[keep], on="L_id", how="left")
