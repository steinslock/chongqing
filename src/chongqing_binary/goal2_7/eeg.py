"""EEG Goal 2.7 event-audited features derived from cached windows."""
...
def extract_eeg_features(config_path: str | Path = "configs/goal2_7/eeg.yaml") -> dict[str, Any]:
    ...
    out_path = ensure_output("artifacts/goal2_7/eeg/feature_manifest.json", config)
...
def _eeg_signal_row(
    ...
    row: dict[str, Any] = {
        ...
        "preprocessing_version": config["eeg"]["preprocessing_version"],
        "task_status": spec.get("task_status", ""),
        "event_validity_status": _event_validity_status(task),
    }
    prefix = "signal"
...
def _erp_features(task: str, windows: np.ndarray, meta: pd.DataFrame, channels: list[str], sfreq: float) -> dict[str, float]:
    ...
    if task in {"oddball"}:
        windows_by_condition = {"oddball_target_only_proxy": windows}
    elif task == "1back":
        return {}
    else:
...
def _attach_split(frame: pd.DataFrame, split: pd.DataFrame, label_col: str) -> pd.DataFrame:
    ...
    if "is_locked_test" in merged.columns and pd.to_numeric(merged["is_locked_test"], errors="coerce").fillna(0).astype(int).any():
        raise ValueError("Goal 2.7 EEG features unexpectedly include pilot-holdout subjects.")
    return merged.sort_values("L_id").reset_index(drop=True)


def _event_validity_status(task: str) -> str:
    if task == "rest":
        return "event_free_rest"
    if task == "oddball":
        return "blocked_target_nontarget_semantics_unconfirmed_target_only_proxy"
    if task == "1back":
        return "blocked_condition_semantics_unconfirmed_generic_signal_only"
    return "unknown"
