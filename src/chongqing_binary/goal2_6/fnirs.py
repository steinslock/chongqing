"""fNIRS Goal 2.6 device-specific traditional features."""
...
def extract_fnirs_features(config_path: str | Path = "configs/goal2_6/fnirs.yaml") -> dict[str, Any]:
    ...
    manifest_path = ensure_output("artifacts/goal2_6/fnirs/feature_manifest.json", config)
...
def _process_yiruid(l_id: str, task: str, path: Path, config: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    ...
    signal.update(_task_delta_features("signal_od", od, marker, task))
...
def _process_bikom(l_id: str, task: str, roles: dict[str, list[Path]], config: dict[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    ...
    signal.update(_task_delta_features("signal_hbo", hbo_values, marker, task))
    signal.update(_task_delta_features("signal_hbr", hbr_values, marker, task))
...
def _task_delta_features(prefix: str, data: np.ndarray, marker: np.ndarray, task: str) -> dict[str, float]:
    if task == "rest":
        return {}
...
def _segments(data: np.ndarray, marker: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if nonzero.size:
        ...
        a = max(1, int(round(n * 0.2)))
        b = max(a + 1, int(round(n * 0.8)))
        baseline = data[:a]
        active = data[a:b]
        recovery = data[b:]
...
def _attach_split(frame: pd.DataFrame, split: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    if pd.to_numeric(merged.get("is_locked_test", 0), errors="coerce").fillna(0).astype(int).any():
        raise ValueError("Goal 2.6 fNIRS features unexpectedly include pilot-holdout subjects.")
